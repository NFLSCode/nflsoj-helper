...
function getUserConfig(page) {
    return JSON.parse(getDOM(page.match(/<td><a href="(\/article\/\d+)">\$helper.config<\/a><\/td>/)[1] + "/edit").getElementById("content").textContent);
}
...
if (/^\/user\/\d+(\/[^e]|$)/.test(domain)) {
    ...
    let config = getUserConfig(document.body.innerHTML);
    ...
    let nameColor = genColorHTML("nobr", "", mainpage[0].innerHTML, config.nameColor),
        backup = getElement("icon")[14].outerHTML, customIcon = `<i class="${config.userIcon}"></i>`;
    ...
    let rank = getElement("ui very basic center aligned table")[0];
    rank.tHead.children[0].innerHTML += "<th>个性签名</th>";
    rank.tHead.children[0].children[1].style.width = "170px";
    for (let i = 0; i < 20; ++i) {
        let td = rank.tBodies[0].children[i], name = td.children[1].innerText, config = getUserConfig(td.children[1].href);
        let firstSign = td.childNodes[9].textContent.match(/translate3d\(0, 0, 0\); \\"\\u003E([\S\s]*?)\\n/);
        console.log(firstSign ? firstSign[1] : "");
        td.innerHTML += `<td>${firstSign ? unescape(firstSign[1].replace(/\\u/g, "%u")).replace(/(?<!\\)\\(?!\\)/g,"") : ""}</td>`;
        td.children[1].innerHTML = genColorHTML("a", `href=${td.children[1].children[0].getAttribute("href")}`, name, getColor(td.childNodes[9].textContent));
    }
}
/******************** rating module ********************/
